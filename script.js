...
function buildCarousel() {
    ...
    items = carousel.querySelectorAll(".carousel-item"); // Актуализация списка!
}
...
function getWinnerName() {
    ...
    items.forEach(item => {
        ...
        const dist = Math.abs(itemCenterX - indicatorCenterX);
        if (dist < minDist) {
            closest = item;
            minDist = dist;
        }
...
document.getElementById("openCase").onclick = function () {
    ...
    animationFrameId = requestAnimationFrame(function animate(currentTime) {
        ...
        if (progress < 1) {
            ...
            const winner = getWinnerName(); // <-- определяем после полной остановки
            showWinner(winner);
            const video = document.getElementById("rouletteVideo");
            video.pause();
...
async function donDigidonCycle() {
    while (participants.length > 1) {
        ...
        await new Promise(r => setTimeout(r, 2000)); // пауза после выбывания
        closeWinner();
        // Скрыть видео после завершения круга
        const video = document.getElementById("rouletteVideo");
        video.pause();
        video.style.display = "none";
